...
import { Layers, Plus, RefreshCw, X, LayoutGrid, Info } from "lucide-react";
...
const AdminCategoriesPage = () => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      {/* Content Section */}
      {isLoading ? (
        <div className="min-h-100 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[3.5rem] flex items-center justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map((category) => (
            <div
              key={category.id}
              className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[2.5rem] p-8 group hover:shadow-2xl hover:shadow-zinc-200/50 transition-all duration-500"
            >
              ...
              <p className="text-zinc-500 text-sm font-medium leading-relaxed line-clamp-2">
                {category.categoryDetails ||
                  "No details provided for this category."}
              </p>
              <div className="mt-8 pt-6 border-t border-zinc-50 dark:border-zinc-800/50 flex justify-between items-center">
                ...
                <Button
                  variant="ghost"
                  className="h-8 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-emerald-600 hover:bg-emerald-50"
                >
                  Edit
                </Button>
              </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[3.5rem] p-20 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-zinc-500 font-medium max-w-sm mx-auto mb-10 leading-relaxed">
            Start by creating your first category to help users find their
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-zinc-900 rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-10">
              <div className="flex items-center justify-between mb-8">
                ...
                <button
                  onClick={() => setIsModalOpen(false)}
                  className="w-10 h-10 rounded-xl bg-zinc-50 dark:bg-zinc-800 flex items-center justify-center text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors"
                >
                  <X size={20} />
                </button>
              </div>
