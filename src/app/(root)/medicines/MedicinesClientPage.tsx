...
interface MedicinesClientPageProps {
  initialMedicines: ApiResponse<IMedicine[]>;
  initialCategories: ApiResponse<IMedicineCategory[]>;
}

const MedicinesClientPage = ({
  initialMedicines,
  initialCategories,
}: MedicinesClientPageProps) => {
  ...
  // Fetching with initial data for SSR
  const { data, isLoading, isFetching } = useAllMedicines(
    {
      searchTerm,
      categoryId,
      minPrice,
  ...
  const handleSearch = React.useCallback(
    (val: string) => updateQuery({ searchTerm: val, page: "1" }),
    [updateQuery],
  );

  const handleCategoryChange = React.useCallback(
    (id: string) => updateQuery({ categoryId: id, page: "1" }),
    [updateQuery],
  );

  const handlePriceChange = React.useCallback(
    (min: string, max: string) => updateQuery({ minPrice: min, maxPrice: max, page: "1" }),
    [updateQuery],
  );

  const handlePopularChange = React.useCallback(
    (val: boolean) => updateQuery({ popular: val ? "true" : null, page: "1" }),
    [updateQuery],
  );
...
export default MedicinesClientPage;
