...
import { useUpdateUserRole, useUpdateUserStatus } from "@/hooks/useAdmin";
...
const UserListTable = ({ users }: UserListTableProps) => {
  ...
  const { mutate: updateRole, isPending: isRoleUpdating } = useUpdateUserRole();
              ...
              >
                ...
                <td className="px-8 py-6 text-right flex items-center gap-2">
                  <Button
                    ...
                    className={cn(
                      "h-10 px-4 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all cursor-pointer border",
                      user.status === "ACTIVE"
                  ...
                  <select
                    ...
                    className="rounded-xl h-10 cursor-pointer bg-white text-emerald-600 border hover:bg-emerald-200 hover:text-zinc-900 px-2"
                  >
